...
function hebcal(b) {
  ...
  const n = "Nisan";
  const e = ["", n, "Iyyar", "Sivan", "Tamuz", "Av", "Elul", "Tishrei", "Cheshvan", "Kislev", "Tevet", "Sh'vat"]
  const o = [[...e, "Adar", n], [...e, "Adar I", "Adar II", n]]
  const u = 5e3
  const f = new Int32Array(2e3)
  const a = -1373428;

  // if "t" is not a number, throw an error that parameter "r" is not a number; it is instead t
  function isNumber(t, r) {
    if ("number" != typeof t || isNaN(t)) throw new TypeError(`param '${r}' not a number: ${t}`);
  }

  function c(t, r, n) {
    if (isNumber(t, "year"), isNumber(r, "month"), isNumber(n, "day"), t < 1) throw new RangeError(`hebrew2abs: invalid year ${t}`);
    let e = n;
    if (r < 7) {
      const n = function (t) {
